...
async function loadScheduleByDate(date: string): Promise<ScheduleRecord[]> {
...
export async function GET(req: NextRequest) {
  try {
    ...
    // 1. スケジュールから当日のPJを取得
    let schedMatched: ScheduleRecord[] = [];
    if (usageDate) {
    ...
    const historyMatched = vendor
      ? expenses.filter(e => {
          const v = (e.vendor_name || '').toLowerCase();
          const nl = vendor.toLowerCase();
          return v && (v.includes(nl) || nl.includes(v));
        })
      : [];
    ...
    let source: PredictResponse['source'] = 'none';
    let confidence = 0;
    ...
    if (!pj_no && !expense_item_code) return NextResponse.json(empty);
