/**
 * saito向け電帳法準拠ファイル名生成
 * フォーマット: {申請年月}_{PJ番号}_{企業名}_{分類}_{店名}_{利用日}.{ext}
 * 例: 2026-04_01260289_岡山マルイ_タクシー代_院庄タクシー_2026-04-16.jpg
 */
export interface FilenameInput {
  apply_month: string | null   // 'YYYY-MM'
  pj_no: string | null
  client_name: string | null   // 企業名（客先）
  category: string | null      // 用途分類（タクシー代/宿泊代/新幹線代等）
  vendor_name: string | null   // 店名/取引先
...
export function buildSaitoFilename(input: FilenameInput, originalFilename: string): string {
  ...
  const cat = sanitize(input.category, 12);
  ...
  return `${apply}_${pj}_${client}_${cat}_${vendor}_${usage}${tail}${ext}`;
}
